...
infra-test:
	...
	@aws --endpoint-url=http://localhost:4566 s3 ls s3://grupo-l-terraform >/dev/null 2>&1 && echo "✅ Bucket S3 'grupo-l-terraform' existe" || echo "❌ Bucket S3 não encontrado"
	@echo ""
	@echo "2️⃣ Testando DynamoDB..."
	@aws --endpoint-url=http://localhost:4566 dynamodb describe-table --table-name GrupoLConstSoftSprint1DynamoDB >/dev/null 2>&1 && echo "✅ Tabela DynamoDB 'GrupoLConstSoftSprint1DynamoDB' existe" || echo "❌ Tabela DynamoDB não encontrada"
	@echo ""
	@echo "3️⃣ Testando IAM Roles..."
	@aws --endpoint-url=http://localhost:4566 iam get-role --role-name ec2_role >/dev/null 2>&1 && echo "✅ IAM Role 'ec2_role' existe" || echo "❌ IAM Role não encontrada"
	@echo ""
	@echo "4️⃣ Testando VPC Security Groups..."
	@aws --endpoint-url=http://localhost:4566 ec2 describe-security-groups --group-names allow-http >/dev/null 2>&1 && echo "✅ Security Group 'allow-http' existe" || echo "❌ Security Group não encontrado"
	@echo ""
	@echo "5️⃣ Testando EC2 Key Pair..."
	@aws --endpoint-url=http://localhost:4566 ec2 describe-key-pairs --key-names grupo-l-key >/dev/null 2>&1 && echo "✅ Key Pair 'grupo-l-key' existe" || echo "❌ Key Pair não encontrado"
	@echo ""
	@echo "6️⃣ Testando EC2 Instance..."
	@aws --endpoint-url=http://localhost:4566 ec2 describe-instances --filters "Name=tag:Name,Values=grupo-l-sprint1" 2>&1 | grep -q "Instances" && echo "✅ EC2 Instance 'grupo-l-sprint1' existe" || echo "❌ EC2 Instance não encontrada"
	@echo ""
	@echo "7️⃣ Testando Cognito (cognito-local)..."
	@aws --endpoint-url=http://localhost:9229 cognito-idp list-user-pools --max-results 10 >/dev/null 2>&1 && echo "✅ Cognito User Pool disponível (cognito-local)" || echo "❌ Cognito não disponível"
	@echo ""
...
infra-debug:
	...
	@aws --endpoint-url=http://localhost:4566 s3 ls 2>&1 || echo "Erro ao listar S3"
	@echo ""
	@echo "📦 Listando todas as tabelas DynamoDB:"
	@aws --endpoint-url=http://localhost:4566 dynamodb list-tables 2>&1 || echo "Erro ao listar DynamoDB"
	@echo ""
	@echo "📦 Listando todos os IAM roles:"
	@aws --endpoint-url=http://localhost:4566 iam list-roles 2>&1 | head -20 || echo "Erro ao listar IAM"
	@echo ""
	@echo "📦 Listando todos os security groups:"
	@aws --endpoint-url=http://localhost:4566 ec2 describe-security-groups 2>&1 | head -20 || echo "Erro ao listar Security Groups"
	@echo ""
	@echo "📦 Listando todos os key pairs:"
	@aws --endpoint-url=http://localhost:4566 ec2 describe-key-pairs 2>&1 || echo "Erro ao listar Key Pairs"
	@echo ""
	@echo "📦 Listando todas as instâncias EC2:"
	@aws --endpoint-url=http://localhost:4566 ec2 describe-instances 2>&1 | head -20 || echo "Erro ao listar EC2"
